...
using Hansom.Infrastructure.Observability;
using Hansom.Tests.Domain;
using Hansom.Tests.Domain.Envelope;

namespace Hansom.Tests.Infrastructure.Observability;
...
public sealed class ObservabilityAttemptObserverTests : IDisposable
{
    ...
    public ObservabilityAttemptObserverTests()
    {
        ...
        {
            ShouldListenTo = source => source.Name == HansomDiagnostics.SourceName,
        ...
        _meterListener = new MeterListener
        {
            ...
            {
                if (instrument.Meter.Name == HansomDiagnostics.MeterName
                    && instrument.Name == "hansom.failures")
                {
        ...
        {
            if (instrument.Name == "hansom.failures")
            {
    ...
    public void failing_attempt_emits_an_activity_with_error_status_and_increments_failures_counter()
    {
        ...
        Assert.Equal("Hansom.Handler.Attempt", activity.OperationName);
    ...
    public void activity_carries_message_type_destination_attempt_and_envelope_id_as_tags()
    {
        ...
        Activity activity = Assert.Single(_activities);
        Assert.Equal(typeof(ChargePayment).FullName, activity.GetTagItem("hansom.message.type"));
        Assert.Equal(envelope.Destination.Value.ToString(), activity.GetTagItem("hansom.destination")?.ToString());
        Assert.Equal(envelope.Attempts.Value, activity.GetTagItem("hansom.attempt"));
        Assert.Equal(envelope.Id.Value.ToString(), activity.GetTagItem("hansom.envelope.id")?.ToString());
    }
