...
public sealed class MiddlewareCatalogTests
{
    ...
    private static Envelope EnvelopeFactoryEnvelope() =>
        Hansom.Tests.Domain.Envelope.EnvelopeFactory.Create(
            message: new Hansom.Domain.Messaging.ValueObjects.Message(new ChargePayment(1)),
            messageType: Hansom.Domain.Messaging.MessageTypeNaming.For(typeof(ChargePayment)));
}
