...
using Hansom.Tests.Application.Mediator;

namespace Hansom.Tests.Application.Sagas;
...
public sealed class SagaConversationTests
{
    ...
    public async Task miss_without_not_found_throws_saga_instance_not_found()
    {
        ...
        IMessageBus bus = new Hansom.Application.Mediator.Mediator(catalog, sagas: new InMemorySagaStore());
    ...
    public async Task throwing_start_persists_no_row_and_publishes_no_cascades()
    {
        ...
        IMessageBus bus = new Hansom.Application.Mediator.Mediator(catalog, cascades, sagas: store);
    ...
    public async Task throwing_handle_leaves_the_previous_row_unchanged()
    {
        ...
        IMessageBus bus = new Hansom.Application.Mediator.Mediator(catalog, sagas: store);
    ...
    public async Task handle_retry_reloads_the_previous_snapshot()
    {
        ...
        IMessageBus bus = new Hansom.Application.Mediator.Mediator(catalog, executor: new Executor(policies), sagas: store);
    ...
    public async Task start_cascades_are_published_and_the_saga_instance_is_not()
    {
        ...
        catalog.Scan(typeof(ConversationSaga));
        var cascades = new RecordingCascadePublisher();
        IMessageBus bus = new Hansom.Application.Mediator.Mediator(catalog, cascades, sagas: new InMemorySagaStore());
    ...
    {
        IMessageBus bus = new Hansom.Application.Mediator.Mediator(new HandlerCatalog(), sagas: new InMemorySagaStore());
    ...
    private static IMessageBus Bus(ISagaStore store)
    {
        var catalog = new HandlerCatalog();
        catalog.Scan(typeof(ConversationSaga));
        return new Hansom.Application.Mediator.Mediator(catalog, sagas: store);
    }
}

public sealed record ConversationStarted([property: Hansom.Domain.Sagas.SagaIdentity] int Id);

public sealed record ConversationPaid([property: Hansom.Domain.Sagas.SagaIdentity] int Id);

public sealed record ConversationTimeout([property: Hansom.Domain.Sagas.SagaIdentity] int Id);

public sealed record ConversationWork(int Id);

public sealed record EmptyIdStarted([property: Hansom.Domain.Sagas.SagaIdentity] string Id);
