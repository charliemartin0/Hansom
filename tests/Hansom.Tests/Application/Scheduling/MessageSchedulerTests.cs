...
namespace Hansom.Tests.Application.Scheduling;
...
public sealed class MessageSchedulerTests
{
    ...
    public async Task a_one_minute_timeout_can_be_played_now()
    {
        ...
        var bus = new Hansom.Application.Mediator.Mediator(catalog, executor: executor, hold: hold);
    ...
    public async Task invoke_of_a_timeout_type_runs_now_and_does_not_park()
    {
        ...
        var catalog = new HandlerCatalog();
        catalog.Scan(typeof(PlayedTimeoutHandler));
        IMessageBus bus = new Hansom.Application.Mediator.Mediator(catalog, hold: hold);
    ...
    {
        IMessageBus bus = new Hansom.Application.Mediator.Mediator(new HandlerCatalog());
    ...
    {
        var hold = new InMemoryScheduledEnvelopeHold();
        IMessageBus bus = new Hansom.Application.Mediator.Mediator(new HandlerCatalog(), hold: hold);
    ...
    public async Task publish_zero_delay_timeout_enqueues_immediately()
    {
        ...
        IMessageBus bus = new Hansom.Application.Mediator.Mediator(
            new HandlerCatalog(),
            hold: hold,
            enqueuer: enqueuer);
    ...
    public async Task throwing_handler_parks_no_timeout()
    {
        ...
        IMessageBus bus = new Hansom.Application.Mediator.Mediator(catalog, cascades, hold: hold);
    ...
    private static Envelope TimeoutEnvelope(DateTimeOffset? deliverBy = null, int orderId = 1) =>
        Hansom.Tests.Domain.Envelope.EnvelopeFactory.Create(
            message: new Hansom.Domain.Messaging.ValueObjects.Message(new OrderTimeout(orderId)),
            messageType: Hansom.Domain.Messaging.MessageTypeNaming.For(typeof(OrderTimeout)),
