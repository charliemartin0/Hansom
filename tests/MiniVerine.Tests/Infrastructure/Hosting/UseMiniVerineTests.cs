...
/// <summary>
/// Prove-with for the DX slice: UseMiniVerine must actually consume the options —
/// scan the configured handler assemblies and apply the fluent routing registrations
/// — so MiniVerine works as a library through its public surface.
/// </summary>
public sealed class UseMiniVerineTests
{
    [Fact]
    public void use_miniverine_scans_handler_assemblies()
    {
        var builder = Host.CreateApplicationBuilder();
        builder.UseMiniVerine(options => options.HandlerAssemblies.Add(typeof(ScannedCommandHandler).Assembly));
